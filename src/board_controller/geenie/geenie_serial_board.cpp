...
GeenieSerialBoard::GeenieSerialBoard (struct BrainFlowInputParams params, int board_id)
...
GeenieSerialBoard::~GeenieSerialBoard ()
{
    skip_logs = true;
    release_session ();
}

int GeenieSerialBoard::open_port ()
{
...
int GeenieSerialBoard::config_board (std::string config, std::string &response)
{
    if (!initialized)
    {
...
int GeenieSerialBoard::send_to_board (const char *msg)
{
...
int GeenieSerialBoard::send_to_board (const char *msg, std::string &response)
{
    int length = (int)strlen (msg);
    safe_logger (spdlog::level::debug, "sending {} to the board", msg);
    int res = serial->send_to_serial_port ((const void *)msg, length);
...
std::string GeenieSerialBoard::read_serial_response ()
{
...
int GeenieSerialBoard::set_port_settings ()
{
...
int GeenieSerialBoard::status_check ()
{
    unsigned char buf[1];
    int count = 0;
    int max_empty_seq = 5;
...
int GeenieSerialBoard::prepare_session ()
{
    if (initialized)
    {
...
int GeenieSerialBoard::start_stream (int buffer_size, const char *streamer_params)
{
    if (is_streaming)
    {
...
int GeenieSerialBoard::stop_stream ()
{
    if (is_streaming)
    {
        keep_alive = false;
...
int GeenieSerialBoard::release_session ()
{
    if (initialized)
    {
        if (is_streaming)

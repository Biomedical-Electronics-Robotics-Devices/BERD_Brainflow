...
class FBCSP(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y=None):
        ...
        for i in range(len(filtered)):
            csp = CSP(**self.kwargs)
            self.csps.append(csp.fit(filtered[i], y))
        return self
    ...
    def transform(self,X):
        ...
        transformed = []
        for i in range(len(filtered)):
            features = self.csps[i].transform(filtered[i])
            features = features.T
            transformed.append(features)
        transformed = np.array(transformed)

        # Assuming that every CSP is fitted with the same number of components
        transformed = transformed.reshape((
            len(transformed)*self.csps[0].n_components, 
            transformed.shape[-1]))
